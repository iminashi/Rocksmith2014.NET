...
let roundTrip (vocals: ResizeArray<XML.Vocal>) = async {
...
let tests =
    testList "Vocals Conversion Tests" [
        ...
        testAsync "Lyrics round trip conversion truncates long lyric" {
            let source = ResizeArray(seq {
                XML.Vocal(5000, 100, "This line of lyrics is more than 48 characters when encoded in UTF8.") })
