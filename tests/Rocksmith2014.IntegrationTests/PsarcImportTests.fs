...
let [<Literal>] ImportDir = "./psarc/import"
...
[<Tests>]
let tests =
    testSequenced <| testList "PSARC Import Tests" [
        testAsync "PC PSARC can be imported" {
            if Directory.Exists ImportDir then Directory.Delete(ImportDir, true)
            Directory.CreateDirectory(ImportDir) |> ignore

            let! project, path = PsarcImporter.import ignore "./psarc/test_p.psarc" ImportDir
        ...
        testCase "Project files were created" <| fun _ ->
            ...
            |> List.iter (fun arr ->
                ...
                Expect.isTrue (File.Exists (Path.Combine(ImportDir, file))) $"Arrangement file {file} exists")

            Expect.isTrue (File.Exists (Path.Combine(ImportDir, "integrationtest.wem"))) "Main audio file exists"
            Expect.isTrue (File.Exists (Path.Combine(ImportDir, "integrationtest_preview.wem"))) "Preview audio file exists"
            Expect.isTrue (File.Exists (Path.Combine(ImportDir, "lyrics.dds"))) "Custom lyrics art file exists"
            Expect.isTrue (File.Exists (Path.Combine(ImportDir, "lyrics.glyphs.xml"))) "Custom lyrics glyphs file exists"
            Expect.isTrue (File.Exists (Path.Combine(ImportDir, "cover.dds"))) "Cover art file exists"
    ]
