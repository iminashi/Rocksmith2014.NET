...
/// Returns the official (artist name) sort value for the string if found,
/// otherwise removes English articles from the beginning of the string.
let useOfficialSortValueIfFound (input: string) =
    input
    |> tryGetOfficialArtistNameSortValue
    |> Option.defaultWith (fun () -> removeArticles input)
    |> sortField

[<RequireQualifiedAccess>]
type FieldType =
    | ArtistName of string
    | Title of string
    | AlbumName of string

/// Converts a field into a sort field.
let convertToSortField (input: FieldType) =
    match input with
    | FieldType.ArtistName artist ->
        artist
        |> useOfficialSortValueIfFound
    | FieldType.Title title ->
        title
        |> removeArticles
        |> sortField
    | FieldType.AlbumName album ->
        album
