...
type Entry =
    ...
    static member Read (reader: IBinaryReader) (index: int32) =
        { ID = index
    ...
    static member CreateProto (namedEntry: NamedEntry) (zBegin: uint32) =
        { NameDigest = Cryptography.md5Hash namedEntry.Name
          ZIndexBegin = zBegin
          Length = uint64 namedEntry.Data.Length
          // Will be set later:
          Offset = 0UL
          ID = 0 }
